...
import { useFavoriteStore } from '@/features/favorite/stores/favorite.store'
...
const FavoritesPage = () => {
  ...
  const dataResult = useQuery({
    queryKey: ['favorite', searchParams.toString()],
    queryFn: async () => {
      const data = await getFavorites(searchParams.toString())
      const comics = await Promise.all(
        data?.data?.results?.map(async (favorite) => {
          const response = await otruyenGetComicApi(favorite.data_id)
        ...
        {dataResult?.data && dataResult?.data?.comics?.length > 0 && (
          <div className="grid gap-4 gap-y-8 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {dataResult?.data?.comics?.map((item) => {
              return (
                <ComicCard key={item?.slug} data={item as unknown as IComic} />
              )
            })}
          </div>
        )}
